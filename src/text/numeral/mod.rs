use crate::{
    data::{memory::PhantomData, FromStr},
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> format::Result {
        match self {
            ...
            Self::InvalidRadix(radix) => {
                write!(f, "invalid radix: {}, must be between 2 and 36", radix)
            }
...
pub trait NumericParser: Sized {
    ...
    fn parse(s: &str) -> Result<Self, ParseNumberError> {
        ...
        let (sign, s, format) = detect_number_format(s)?;
        ...
        match format {
            ...
            NumberFormat::Hexadecimal => {
                ...
                Self::from_str_radix(&normalized, 16)
            }
            NumberFormat::Octal => {
                let normalized = match sign {
                    Some('-') => format!("-{}", s),
                ...
                Self::from_str_radix(&normalized, 8)
            }
            NumberFormat::Binary => {
                let normalized = match sign {
                    Some('-') => format!("-{}", s),
                ...
                Self::from_str_radix(&normalized, 2)
            }
            NumberFormat::Scientific => {
                parse_scientific(s).and_then(|f| Self::from_str_radix(&f.to_string(), 10))
            }
...
fn detect_number_format(s: &str) -> Result<(Option<char>, &str, NumberFormat), ParseNumberError> {
    if s.is_empty() {
        return Err(ParseNumberError::EmptyString);
    }

    let (sign, body) = if let Some(first) = s.chars().next() {
        if first == '-' || first == '+' {
            (Some(first), &s[1..])
        } else {
            (None, s)
    ...
    if body.len() >= 2 && body.starts_with('0') {
        match body.chars().nth(1).unwrap() {
            'x' | 'X' => return Ok((sign, &body[2..], NumberFormat::Hexadecimal)),
            'o' | 'O' => return Ok((sign, &body[2..], NumberFormat::Octal)),
            'b' | 'B' => return Ok((sign, &body[2..], NumberFormat::Binary)),
            _ => {}
        }
    }

    if s.contains(['e', 'E']) && !s.starts_with(['e', 'E']) {
        return Ok((None, s, NumberFormat::Scientific));
    }

    Ok((None, s, NumberFormat::Decimal))
}
...
fn parse_scientific(s: &str) -> Result<f64, ParseNumberError> {
    ...
    let base = parts[0]
        .parse::<f64>()
        .map_err(|_| ParseNumberError::InvalidFormat(format!("invalid base: {}", parts[0])))?;

    let exp = parts[1]
        .parse::<i32>()
        .map_err(|_| ParseNumberError::InvalidFormat(format!("invalid exponent: {}", parts[1])))?;
            ...
            fn from_str_radix(s: &str, radix: u8) -> Result<Self, ParseNumberError> {
                ...
                if s.is_empty() {
                    return Err(ParseNumberError::InvalidFormat(
                        "only a sign character found".to_string(),
                    ));
                }
            ...
            fn from_str_radix(s: &str, radix: u8) -> Result<Self, ParseNumberError> {
                if radix != 10 && !s.contains(['e', 'E']) {
                    ...
                    <$t as FromStr>::from_str(s).map_err(|_| {
                        ParseNumberError::InvalidFormat(format!(
                            "failed to parse '{}' as {}",
                            s,
                            stringify!($t)
                        ))
...
impl AutoNumber {
    ...
    pub fn as_int(&self) -> Result<i64, ParseNumberError> {
        match self {
            ...
            _ => Err(ParseNumberError::TypeError(
    ...
    pub fn as_float(&self) -> Result<f64, ParseNumberError> {
        match self {
            ...
            Self::F32(v) => {
                let representation = format!("{}", v);
                Ok(representation.parse::<f64>().unwrap_or(*v as f64))
            }
            Self::F64(v) => Ok(*v),
        }
    }
...
impl AutoNumberParser {
    ...
    pub fn parse(&self, s: &str) -> Result<AutoNumber, ParseNumberError> {
        ...
        if s.contains('.') || s.contains(['e', 'E']) {
            let prefers_f64 = if let Some((_, exponent)) = s.split_once(|c| c == 'e' || c == 'E') {
                exponent
                    .parse::<i32>()
                    .map(|value| value.abs() >= 38)
                    .unwrap_or(false)
            } else {
                false
            };
            ...
            return Err(ParseNumberError::InvalidFormat(format!(
