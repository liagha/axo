...
//!
//! `core` is a high-performance library for number-to-string and
...
//!
//! `core` is well-tested, and has been downloaded more than 50 million
//! times and currently has no known errors in correctness. `core`
...
//! ```
//!
//! All `core` parsers are validating, they check the that entire
...
//! `core`, you may use the following approach:
//!
//! ```rust
...
//!
//! `core` provides two main levels of configuration:
...
//! # Safety Guarantees
//!
//! There is no non-trivial unsafe behavior in `core` itself,
//! however, any incorrect safety invariants in our parsers and writers
//! ([`parse-float`], [`parse-integer`],
//! [`write-float`], and [`write-integer`]) could cause those
