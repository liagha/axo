...
for Binding<Target, Value, Type>
{
    fn format(&self, config: Stencil) -> Stencil {
        let mut stencil = config.clone().new("Binding")
...
impl<'show, Target: Show<'show>, Member: Show<'show>> Show<'show> for Aggregate<Target, Member> {
    fn format(&self, config: Stencil) -> Stencil {
        config.clone().new("Aggregate")
            .field("target", self.target.format(config.clone()))
            .field("members", self.members.format(config.clone()))
    }
}

impl<'show, Target: Show<'show>, Parameter: Show<'show>, Body: Show<'show>, Output: Show<'show>>
Show<'show> for Function<Target, Parameter, Body, Output>
{
    fn format(&self, config: Stencil) -> Stencil {
        config.clone().new("Function")
...
impl<'show, Operator: Show<'show>, Operand: Show<'show>> Show<'show>
for Unary<Operator, Operand>
{
    fn format(&self, config: Stencil) -> Stencil {
        config.clone().new("Unary")
            .field("operator", self.operator.format(config.clone()))
...
for Index<Target, Member>
{
    fn format(&self, config: Stencil) -> Stencil {
        config.clone().new("Index")
            .field("target", self.target.format(config.clone()))
            .field("members", self.members.format(config.clone()))
    }
}
...
impl<'show, Target: Show<'show>, Member: Show<'show>> Show<'show>
for Invoke<Target, Member>
{
    fn format(&self, config: Stencil) -> Stencil {
        config.clone().new("Invoke")
            .field("target", self.target.format(config.clone()))
            .field("members", self.members.format(config.clone()))
    }
}
